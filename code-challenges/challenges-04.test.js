...
const isCapitalized = (str) => {
  ...
  // match returns something or nothing.
  return str.match(regex) ? str.match(regex) : arr;
};
...
const matchMonth = (input) => {
  let regex1 = /[O-o]ct\b/;
  let regex2 = /\b[O-o]ctober/; // \b is important
  let stringInput = input.toString();
  if (stringInput.match(regex1)) {
    return true;
  } else if (stringInput.match(regex2)) {
    return true;
  } else {
    return false;
  }
...
const noPunctuation = str => {
  let regex = /\w+\s/g;
  return str.match(regex); // returns an array
};
